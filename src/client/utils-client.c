...
{
    _sys_log(__VERBOSE_FUNC,"get_next_macs(%i)\n",mode);
    ...
    _sys_log(__VERBOSE_STD,"get_next_macs(Self: %.2x:%.2x:%.2x:%.2x:%.2x:%.2x, Othr: %.2x:%.2x:%.2x:%.2x:%.2x:%.2x)\n",
...
{
    _sys_log(__VERBOSE_FUNC,"advance_mac(%p,%p,%i)\n",conn,macs,who);
